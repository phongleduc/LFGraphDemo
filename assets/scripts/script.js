$(function () {
    ...
    var dragHandler = {
                    ...
                    for (var i = 0; i < localGraphObjects.length; i++) {
                        ...
                        switch (localGraphObjects[i]) {
                            case "0":
                                ...
                                break;
                            case "5":
                                var tage = $(dropArea).find('.chartBarPrecipitation')[0];
                                tage.id = "chart-Bar-Precipitation-" + dragHandler.objectIndex++;
                                barChartPreHandler.renderBarChart(tage.id);
                                break;
                            case "1":
                            ...
                            case "3":
                                ...
                                break;
                            case "6":
                                var tagetPie = $(dropArea).find('.chartPiePrecipitation')[0];
                                tagetPie.id = "chart-Pie-Precipitation-" + dragHandler.objectIndex++;
                                pieChartHandler.renderPieChart(tagetPie.id);
                                break;
                            case "7":
                                var tagetArea = $(dropArea).find('.chartAreaPrecipitation')[0];
                                tagetArea.id = "chart-Area-Precipitation-" + dragHandler.objectIndex++;
                                areaChartHandler.renderAreaChart(tagetArea.id);
                                break;
                            default:
                                break;
                        }
                ...
                receive: function (event, ui) {
                    ...
                    switch (check) {
                        ...
                        case "bar-chart-precipitation":
                            dragHandler.renderBarChart(target);
                            break;
                        case "pie-chart-precipitation":
                            dragHandler.renderPieChart(target);
                            break;
                        case "area-chart-precipitation":
                            dragHandler.renderAreaChart(target);
                            break;
                        default:
                            break;
                    }
        ...
        renderGau: function (className, target) {
            var gra = $(target).find(className)[0];//plot-chart
            gra.id = "air-temperature-" + dragHandler.objectIndex;
            //get api
            ...
            graphCurrentWeather.init(gra.id, temp, title);
        },
        renderPieChart: function (target) {
            var gra = $(target).find(".chartPiePrecipitation")[0];
            gra.id = "chart-Pie-Precipitation-" + dragHandler.objectIndex;
            pieChartHandler.renderChart(gra.id);
        },
        renderBarChart: function (target) {
            var gra = $(target).find(".chartBarPrecipitation")[0];
            gra.id = "chart-Bar-Precipitation-" + dragHandler.objectIndex;
            barChartPreHandler.renderBarChart(gra.id);
        },
        renderAreaChart: function (target) {
            var gra = $(target).find(".chartAreaPrecipitation")[0];
            gra.id = "chart-Area-Precipitation-" + dragHandler.objectIndex;
            areaChartHandler.renderChart(gra.id);
        }

    };
    var pieChartHandler = {
        renderChart: function (idCount) {
            var dataChar2 = [
                ['Heavy Industry', 12], ['Retail', 9], ['Light Industry', 14],
                ['Out of home', 16], ['Commuting', 7], ['Orientation', 9]
            ];
            var plot1 = $.jqplot(idCount, [[['a', 25], ['b', 14], ['c', 7]]], {
    ...
    var areaChartHandler = {
        renderChart: function (idAreaChart) {
            var datas = {
            ...
            var plot3 = $.jqplot(idAreaChart, [datas.rural, datas.urban], {
    ...
    var barChartPreHandler = {
        renderBarChart: function (idCount) {
            //get api
            ...
            var s1 = [2, 3, 9, 12, 5, 8, 1];
            var ticks = ['27/11', '28/11', '29/11', '30/11', '01/12', '02/12', '03/12'];
